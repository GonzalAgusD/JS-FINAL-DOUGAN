...
const applyFilter = (e) => {
  ...
  conteinerProduct.innerHTML = ""; // Limpia el contenedor de productos

  if (miEstado.filtrado) {
    const filteredProducts = miEstado.productos.filter(
      (product) => product.category === miEstado.filtrado
    );
    cargarProduct(filteredProducts);
    miEstado.comienza = 0; // Reinicia el contador de productos mostrados
    return;
  }
  
  // Si no hay un filtro activo, carga los primeros productos
  cargarProduct(miEstado.productos.slice(miEstado.comienza, 1));
}; 
...
const completarCartAction = (confirmMsg, successCallback) => {
  if (!cart.length) return;
  if (window.confirm(confirmMsg)) {
    resetCartItems();
    successCallback(); // Ejecuta el callback de éxito, que ahora será Swal.fire
  }
};
...
const init = () => {
  fetch('JS/producto.json')
    ...
    .then(data => {
      ...
      miEstado.productos = data;
      miEstado.comienza = 0;
      miEstado.limiteDeProduct = miEstado.productos.length;

      cargarProduct(miEstado.productos.slice(miEstado.comienza, 26));
